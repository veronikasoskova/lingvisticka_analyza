...
- Do NOT run `python generate_demo_db.py` on the committed corpus. Tab 2 ships a live full-BKR Stanza run packed as `output/bible_analysis.db.gz` (~38k sentences). `a_paths.ensure_bible_db()` unpacks it to `output/bible_analysis.db` (gitignored). `generate_demo_db.py` would replace those rows with synthetic labels and can store boolean flags as TEXT; with `pandas>=2` that makes Tab 2 `groupby.mean()` charts fail or show NaNs. If the unpacked DB is clobbered, delete `output/bible_analysis.db` and unpack again, or restore `output/bible_analysis.db.gz` from git.
